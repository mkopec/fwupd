...
fu_dell_detect_dock (FuDellSmiObj *smi_obj, guint32 *location)
{
	...
	if (!fu_dell_execute_simple_smi (
		smi_obj, DACI_DOCK_CLASS, DACI_DOCK_SELECT))
		return FALSE;
	if (count_out->ret != 0) {
...
fu_dell_query_dock (FuDellSmiObj *smi_obj, DOCK_UNION *buf)
{
	...
	if (smi_obj->fake_smbios)
		...
		buf->buf = dell_smi_obj_make_buffer_frombios_auto (
		    smi_obj->smi, cbARG3, buf_size);
		if (!buf->buf) {
	...
	if (result != SMI_SUCCESS) {
		if (result == SMI_INVALID_BUFFER) {
			g_debug (
			    "Invalid buffer size, needed %" G_GUINT32_FORMAT,
...
gboolean
fu_dell_toggle_host_mode (FuDellSmiObj *smi_obj, const efi_guid_t guid,
			  int mode)
{
	gint ret;
	ADDR_UNION buf;
