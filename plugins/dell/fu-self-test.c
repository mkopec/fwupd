...
fu_plugin_dell_tpm_func (void)
{
	...
	ret = fu_plugin_open (
	    plugin, PLUGINBUILDDIR "/libfu_plugin_dell.so", &error);
	...
	fu_plugin_dell_inject_fake_data (
	    plugin, (guint32 *)&tpm_out, 0, 0, NULL, FALSE);
	...
	fu_plugin_dell_inject_fake_data (
	    plugin, (guint32 *)&tpm_out, 0, 0, NULL, TRUE);
	ret = fu_plugin_dell_detect_tpm (plugin, &error);
	...
	g_assert_false (
	    fu_device_has_flag (device_alt, FWUPD_DEVICE_FLAG_UPDATABLE));
	...
	ret = fu_plugin_runner_update (
	    plugin, device_alt, NULL, NULL, FWUPD_INSTALL_FLAG_NONE, &error);
	g_assert (!ret);
	g_assert_error (error, FWUPD_ERROR, FWUPD_ERROR_NOT_SUPPORTED);
	g_clear_error (&error);
	...
	/* test override */
	ret = fu_plugin_runner_update (
	    plugin, device_alt, NULL, NULL, FWUPD_INSTALL_FLAG_FORCE, &error);
	g_assert (ret);
	g_assert_no_error (error);
...
fu_plugin_dell_dock_func (void)
{
	...
	fu_plugin_dell_inject_fake_data (
	    plugin, (guint32 *)&out, 0x1234, 0x4321, NULL, FALSE);
	...
	out[1] = 0;
	fu_plugin_dell_inject_fake_data (
	    plugin, (guint32 *)&out, DOCK_NIC_VID, DOCK_NIC_PID, NULL, FALSE);
	fu_plugin_dell_device_added_cb (NULL, NULL, plugin);
	g_assert (device == NULL);
	...
	g_clear_object (&device);
	g_free (buf.record);
	fu_plugin_dell_device_removed_cb (NULL, NULL, plugin);
	...
	memcpy (dock_info->components[1].description,
		"Dock1,PC,TI,IE_Dock,0 :Query 2 1 0 2 0",
		38);
...
main (int argc, char **argv)
{
	...
	g_assert_cmpint (
	    g_mkdir_with_parents ("/tmp/fwupd-self-test/var/lib/fwupd", 0755),
	    ==,
	    0);
